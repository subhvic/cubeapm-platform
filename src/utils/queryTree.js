...
// ---------- Facet filters ----------
// The filters panel and the query bar are one state. The panel starts fully
// ticked because an empty query means "*", so a field with no chip is a field
// with nothing filtered.
//
// A field is expressed whichever way is truer to what the user did, exactly as
// Datadog does it: unticking values builds exclusions, `Only` builds an
// inclusion. Both are read back, so either can be typed by hand.
//
//   include  field:=a            OR-group when there are several
//   exclude  field!=a            AND-group when there are several
//
// Exclusions join with AND, never OR. A record has one value per field, so
// `!=a OR !=b` is true of every record and would filter nothing at all.

const FACET_OPS = { eq: 'include', neq: 'exclude' }
const GROUP_CONNECTOR = { include: 'OR', exclude: 'AND' }

function facetLeafMode(n) {
  return !isGroup(n) && typeof n?.field === 'string' ? (FACET_OPS[n.op] ?? null) : null
}

// A group counts only when every child filters one field the same way, joined
// by the connector that mode requires.
function facetGroupInfo(n) {
  if (!isGroup(n) || !n.children?.length) return null
  const [head, ...rest] = n.children
  const mode = facetLeafMode(head)
  if (!mode) return null
  const want = GROUP_CONNECTOR[mode]
  const ok = rest.every(c =>
    facetLeafMode(c) === mode && c.field === head.field && (c.connector ?? 'AND') === want)
  return ok ? { field: head.field, mode } : null
}

function facetInfo(n) {
  const g = facetGroupInfo(n)
  if (g) return g
  const mode = facetLeafMode(n)
  return mode ? { field: n.field, mode } : null
}

// Map of field → { mode, values } for every field the panel owns.
export function facetStates(nodes) {
  const out = new Map()
  for (const n of nodes ?? []) {
    const info = facetInfo(n)
    if (!info) continue
    const values = isGroup(n) ? n.children.map(c => String(c.value)) : [String(n.value)]
    const prev = out.get(info.field)
    out.set(info.field, {
      mode: info.mode,
      values: new Set([...(prev?.values ?? []), ...values]),
    })
  }
  return out
}

// Replaces whatever the panel owns for `field`. `filter` of null clears it,
// which is what "everything is ticked" looks like in the tree: nothing.
export function setFacetFilter(nodes, field, filter) {
  const list = (nodes ?? []).filter(n => facetInfo(n)?.field !== field)
  if (!filter || filter.values.length === 0) return normalize(list)

  const op = filter.mode === 'include' ? 'eq' : 'neq'
  const conn = GROUP_CONNECTOR[filter.mode]
  const children = filter.values.map((v, i) =>
    setConn({ field, op, value: String(v) }, i === 0 ? null : conn))
  return normalize([...list, children.length === 1 ? children[0] : newGroup(children)])
}

// Drops every facet-owned node — everything ticked again — leaving hand-built
// filters untouched.
export function clearFacets(nodes) {
  return normalize((nodes ?? []).filter(n => !facetInfo(n)))
}

// The chip shape for a desired selection. `prefer` keeps the field expressed
// the way it already was, so unticking a second value extends the exclusion
// rather than flipping the whole field to an inclusion behind the user.
export function facetFilterFor(selected, allValues, prefer = 'exclude') {
  const sel = selected instanceof Set ? selected : new Set(selected)
  if (sel.size === allValues.length) return null
  // Nothing ticked is only expressible as excluding everything.
  if (sel.size === 0) return { mode: 'exclude', values: [...allValues] }
  return prefer === 'include'
    ? { mode: 'include', values: allValues.filter(v => sel.has(v)) }
    : { mode: 'exclude', values: allValues.filter(v => !sel.has(v)) }
}
